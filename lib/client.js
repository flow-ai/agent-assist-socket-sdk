...
var Client = /*#__PURE__*/function (_EventEmitter) {
  ...
  function Client(opts) {
    ...
    _this.on(Client.TRIGGER_WORKFLOWS_OPENING, _this.sendWorkflowsOpening);
    ...
    _this._endpoint = opts.environment === 'qa' ? 'https://flow.dev.aws.lcloud.com/agent-assist-gateway-web' : 'https://app.flow.ai/agent-assist-gateway-web';
    _this._silent = !!opts.silent;
    _this._session = opts.session;
    _this._pageId = opts.pageId;
  ...
  _createClass(Client, [{
    ...
    **/
    ...
    value: function send(message) {
      ...
      if (!this.isConnected) {
        this.emit(Client.ERROR, new _exception["default"]('Could not send the message. The socket connection is disconnected.', 'connection'));
        return;
      }

      if (!message.meta && !message.meta.namespace) {
        this.emit(Client.ERROR, new _exception["default"]('Meta or namespace is not provided for the message', 'message'));
        return;
      }

      this._socket.send(JSON.stringify({
        type: 'message.send',
        payload: _objectSpread(_objectSpread({}, message), this._buildCommonData())
      }));
    }
  }, {
    key: "sendWorkflowsOpening",
    value: function sendWorkflowsOpening(meta) {
      debug('Sending workflows opening %j', {
        meta: meta,
    ...
    value: function _handleConnection(payload) {
      ...
      socket.onclose = function (evt) {
        ...
        if (evt.code === 0) {} else if (evt.code === 1006) {
